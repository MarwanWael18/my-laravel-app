...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="bg-[#FDFDFC] dark:bg-[#0a0a0a] text-[#1b1b18] flex items-center lg:justify-center min-h-screen flex-col">
        <header class="w-full text-sm mb-6 not-has-[nav]:hidden" style="position: absolute; top: 0; backdrop-filter: blur(23px)">
            @if (Route::has('login'))
                    ...
                    <div class="container">
                        ...
                        <div class="collapse navbar-collapse" id="collapsibleNavId">
                            <ul class="navbar-nav m-auto mt-2 mt-lg-0">
                                <li class="nav-item">
                                    <a class="nav-link text-white" href="{{ route('welcome') }}" aria-current="page"
                                        >Home
                                        <span class="visually-hidden">(current)</span></a
                                    >
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link mx-3 text-white" href="{{ route('shop') }}">Shop</a>
                                </li>
                                <li class="nav-item">
        ...
        @foreach ($cart as $item)
            <div class="item-row card rounded-5 mb-3 p-3 container" style="background-color: #181614">
                <div class=" d-flex align-items-center">
                    <img src="{{ asset('storage/' . $item->image) }}" alt="" width="15%" class="rounded-5">
                    <div class="card-body">
                        <h4 class="card-title text-white" style="font-size: 32px">{{ $item->name }}</h4>
                        <p class="card-text" style="color: #F58C26; font-size: 20px">$ {{ $item->price }}</p>
                    </div>

                    <div class="item-row d-flex align-items-center">
                        <button type="button" class="btn btn-sm update-qty rounded-3" data-id="{{ $item->id }}" data-type="minus" style="border: 1px solid #F58C26; font-size: 20px">
                            <i class="fa-solid fa-minus text-white"></i>
                        </button>

                        <span class="qty-label mx-4 text-white" style="font-size: 32px">{{ $item->quantity }}</span>

                        <button type="button" class="btn btn-sm update-qty" data-id="{{ $item->id }}" data-type="plus" style="border: 1px solid #F58C26; font-size: 20px">
                            ...
                            <button type="button" class="btn delete-btn text-white" data-id="{{ $item->id }}">
        ...
        <div class="card rounded-5 mb-3 p-3 container" style="background-color: #181614">
            <div class="card-body d-flex justify-content-between">
                <h4 class="card-title" style="color: #ffffff8f">Title</h4>
                @foreach ($cart as $item)
                <div class="total-price text-white">
                    <span id="grand-total" style="font-size: 22px; font-weight: 800">${{ $cart->sum(fn($i) => $i->price * $i->quantity) }}</span>
                </div>
                @endforeach
            </div>
            <div class="d-flex mt-2">
                <button class="btn rounded-5 w-100 mx-3" style="background-color: #F58C26">Checkout</button>
            </div>
        </div>
...
$(document).on('click', '.update-qty', function() {
    ...
    let container = $(this).closest('.item-row');
    let qtyLabel = container.find('.qty-label');

    $.ajax({
        url: `/cart/${itemId}/${type}`,
        type: "POST",
    ...
    if (response.was_deleted) {
        ...
        qtyLabel.text(response.new_qty);
    }
}
    });
});
...
$(document).on('click', '.delete-btn', function() {
    let btn = $(this);
    let id = btn.data('id');
    let container = btn.closest('.item-row');
